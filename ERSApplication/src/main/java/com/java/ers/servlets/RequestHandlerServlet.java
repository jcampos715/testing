...
public class RequestHandlerServlet extends HttpServlet {
	...
	protected final static Logger tracker = Logger.getLogger(RequestHandlerServlet.class);
		...
		if (type.equalsIgnoreCase("login")) {
			...
			if (user == null) {
				...
				if (reDao.insertReimbursement(re)) {
					...
					//log submission of new ticket
					tracker.info(user.getUserName()+" submitted a new ticket");
					//System.out.println("logged ticket change:submit");
			...
			if (user == null) {
				...
				if(cat.equalsIgnoreCase("approve")) { statusId = sDao.getStatusId(Constants.STATUS_APPROVED);
					//log approval of new ticket
					tracker.info(user.getUserName()+" approved ticket Id: "+id);
					//System.out.println("logged ticket change: approved");
				}
				else { statusId = sDao.getStatusId(Constants.STATUS_DENINED);
					//log denial of new ticket
					tracker.info(user.getUserName()+" denied ticket Id: "+id);
					//System.out.println("logged ticket change: denied");
				}
